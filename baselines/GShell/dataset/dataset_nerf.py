...
def _resolve_mask_path(base_dir, frame):
...
class DatasetNERF(Dataset):
    ...
    def _preload_cache_key(cls, cfg_path, FLAGS):
        return (
            ...
            bool(FLAGS.use_depth),
            bool(FLAGS.use_depth_2nd_layer),
        )
    ...
    def _parse_frame(self, cfg, idx):
        ...
        camera_angle_x = frame.get('camera_angle_x', cfg.get('camera_angle_x'))
        if camera_angle_x is None:
            raise KeyError(f"Frame {idx} is missing camera_angle_x")
        ...
        mask_path = _resolve_mask_path(self.base_dir, frame)
        if not os.path.exists(mask_path):
            raise FileNotFoundError(f"Mask target is missing: {mask_path}")
        ...
        if self.FLAGS.use_depth_2nd_layer:
            ...
            if not os.path.exists(invdepth_second_path):
                raise FileNotFoundError(
                    f"Second-layer depth is enabled but target is missing: {invdepth_second_path}"
                )
    ...
    def __getitem__(self, itr):
        ...
        if invdepth_second is not None and (
            invdepth_second.shape[1] != iter_res[0] or invdepth_second.shape[2] != iter_res[1]
        ):
