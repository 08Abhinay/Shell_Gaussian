...
def neus2_normalization(
    ...
    if len(training_frames) != len(TRAIN_INDICES):
...
def validate_neus2_scene(
    ...
    if (
        ...
        errors.append("NeuS2 source scene path changed")
    source_transforms = source_scene / "transforms.json"
    if manifest.get("source_transforms_sha256") != sha256_file(
        source_transforms
    ):
        errors.append("source transforms.json changed after NeuS2 preparation")
