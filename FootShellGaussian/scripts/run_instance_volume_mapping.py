...
def _audit_mapping(mapping: InstanceVolumeMap) -> dict[str, Any]:
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    for offset, name in enumerate(names, start=1):
        shoe_started = time.monotonic()
        print(f"[11-C {offset}/{len(names)}] {name}: loading", flush=True)
        try:
            mapping = load_instance_volume_map(
                canonical, batch_root / name, containment_root / name
            )
            print(f"[11-C {offset}/{len(names)}] {name}: auditing", flush=True)
            report = _audit_mapping(mapping)
            status = "mapping_valid"
            error = None
        ...
        elapsed = time.monotonic() - shoe_started
        print(
            f"[11-C {offset}/{len(names)}] {name}: {status} ({elapsed:.1f}s)",
            flush=True,
        )
        summary = {
            "schema_version": 1,
            ...
            "status": (
                ...
                and all(value["status"] == "mapping_valid" for value in results.values())
                else "running_or_failed"
            ),
            "total": len(names),
            "completed": len(results),
            "successful": sum(
                value["status"] == "mapping_valid" for value in results.values()
            ),
            "failed": sum(
                value["status"] == "failed_11_c" for value in results.values()
            ),
            "results": results,
        }
        _write_json_atomic(output_root / "mapping_summary.json", summary)

    accepted = all(value["status"] == "mapping_valid" for value in results.values())
    summary["status"] = "mapping_valid" if accepted else "failed_11_c"
    _write_json_atomic(output_root / "mapping_summary.json", summary)
    print(
